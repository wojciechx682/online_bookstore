<?php
	...
	if(((isset($_POST['stare_haslo_edit'])) && (isset($_POST['nowe_haslo_edit'])) && (isset($_POST['powtorz_haslo_edit']))) && ((!empty($_POST['stare_haslo_edit'])) && (!empty($_POST['nowe_haslo_edit'])) && (!empty($_POST['powtorz_haslo_edit'])))) 
	{
		...
		query("SELECT haslo FROM klienci WHERE id_klienta='$id'", "verify_password", $id); // ta funkcja ustawia zmienna sesyjna $_SESSION['stare_haslo'] ktora przechowuje haslo z BD // CZY TO BEZPIECZNE ABY ZMIENNA SESYJNA PRZECHOWYWALA ZAHASHOWANE HASLO ?
