...
//		}
		...
		if(!filter_var($email_s, FILTER_VALIDATE_EMAIL)) // email nie przeszedł walidacji
		{						
			$validation = false;
...
//			array_push($values, $_SESSION["id"]);
            ...
            $user_data = [$imie, $nazwisko, $email, $telefon, $_SESSION["id"]];

			//$id = $_SESSION['id'];

			echo query("UPDATE klienci SET imie='%s', nazwisko='%s', email='%s', telefon='%s' WHERE id_klienta='%s'", "", $user_data);
