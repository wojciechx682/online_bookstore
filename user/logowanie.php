		...
		if ( (filter_var($email, FILTER_VALIDATE_EMAIL) == false) || $email == null || ($email != $_POST["email"]) ) {
            ...
            if (isset($_SESSION['blad'])) { // ✓ niepoprawne dane logowania - nie znaleziono takiego KLIENTA (email) !, próba znalezienia takiego pracownika;
                ...
                query("SELECT pr.id_pracownika, pr.haslo, pr.imie, pr.nazwisko, pr.telefon, pr.email, pr.stanowisko, pr.adres_id,
                         ...
                         AND pr.email='%s'", "log_in", $email) ; // problem implementacyjny --> ✓ zmodyfikować funkcję log_in ! // ✓ dodac instrukcję warunkową (if);
            }

            header('Location: ___zaloguj.php'); exit(); // zmienna $_SESSION["blad"] jest ustawiona, podano adres e-mail który nie należy ani do klienta, ani do pracownika;
