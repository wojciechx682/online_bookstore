    ...
    if ( (isset($_POST["name"]) &&
		...
		if (!filter_var($email_s, FILTER_VALIDATE_EMAIL) || ($email_s !== $email)) {
            ...
            if ($email != $_SESSION["email"]) {
                ...
                query("SELECT email FROM clients WHERE BINARY email='%s' UNION SELECT email FROM employees WHERE BINARY email='%s'", "checkEmail", [$email, $email]);
		...
		if ($valid) {
			...
			$updateSuccessful = query("UPDATE clients SET imie='%s', nazwisko='%s', email='%s', telefon='%s' WHERE id_klienta='%s'", "", $user_data); // true / false;
