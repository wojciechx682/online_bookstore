...
<?php
	...
	if( isset($_POST['id_ksiazki']) &&     // insert book into shopping cart;
        isset($_POST['koszyk_ilosc']) &&   // dane pochodzące z formularza (index.php / book.php) po kliknięciu "Dodaj do koszyka"
        ! empty($_POST['id_ksiazki']) &&   // "23" - id książki ;
        ! empty($_POST['koszyk_ilosc'])    //  "1" - ilość egzemplarzy, która ma zostać dodana do koszyka ;
      )
	{
		$id_klienta = filter_var($_SESSION['id'], FILTER_SANITIZE_NUMBER_INT);

		/*$id_ksiazki = $_POST['id_ksiazki']; // user może wpr. dowolną wartość zmieniając atrybut "value" w <input type hidden> ...
		...
		$id_ksiazki = filter_input(INPUT_POST, "id_ksiazki", FILTER_SANITIZE_NUMBER_INT); // "35" ;
			// to remove any non-numeric characters. This filter will leave only the numeric characters.
		$ilosc = filter_input(INPUT_POST, "koszyk_ilosc", FILTER_SANITIZE_NUMBER_INT);    // "5" ;
		...
		query("SELECT * FROM koszyk WHERE id_klienta = '%s' AND id_ksiazki = '%s'", "cart_verify_book", [$id_klienta, $id_ksiazki]);
		...
		if($_SESSION['book_exists']) {                         // if boox exists, update book quantity in shopping_cart ;
