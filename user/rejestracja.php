<?php
	...
	if (isset($_POST["email"]) && !empty($_POST["email"]) &&
        ...
        */
		...
		query("SELECT id_klienta FROM clients WHERE email='%s'", "registerVerifyEmail", $email_sanitized);
		...
		if ($_SESSION["valid"]) {
            ...
            if( isset($_SESSION["udanarejestracja"]) && // jeśli udało się wstawić wiersz do tabeli "adres";
                ...
                $insertSuccessful = query("INSERT INTO clients (id_klienta, imie, nazwisko, email, telefon, haslo, adres_id) VALUES (NULL, '%s', '%s', '%s', '%s', '%s', '%s')", "", $user);
