<?php
	...
	if ( isset($_POST["comment"]) && !empty($_POST["comment"]) &&
		...
		query("SELECT id_komentarza, id_ksiazki, id_klienta, tresc FROM comments WHERE id_klienta = '%s' AND id_ksiazki = '%s'", "verifyRateExists", $comment_data);
		...
		if (!empty($_SESSION["rate_exists"])) {
			...
			$commentInserted = query("INSERT INTO comments (id_komentarza, id_klienta, id_ksiazki, tresc) VALUES (NULL, '%s', '%s', '%s')", "", $comment_data); // dodanie nowego komentarza
			...
			$ratingUpdated = query("UPDATE books 
				  INNER JOIN (SELECT AVG(ratings.ocena) avg_score, ratings.id_ksiazki 
				  FROM ratings 
				  GROUP BY ratings.id_ksiazki) x ON x.id_ksiazki = books.id_ksiazki 
				  SET rating = x.avg_score 
				  WHERE books.id_ksiazki = '%s'", "", $comment_data[1]);
