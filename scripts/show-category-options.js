...
function showCategoryOptions(id) {   // admin/categories.php - kliknięcie "Zarządzaj";   -->  ✓ wyświetlenie / ukrycie listy opcji ;
    ...
    let categoryActionOptions = document.querySelectorAll('div.category-action'); // kolekcja pojemników z przyciskiem "Zarządzaj";

    for(let i = 1; i < categoryActionOptions.length; i++) {    // dla każdego <div>'a   -->   <div class="order-action">

        let categoryActionButton = categoryActionOptions[i].querySelector('.category-action-button'); // przycisk "Zarządzaj" - </div> ;
        ...
        if(categoryActionButton.id !== id) {
            ...
            let icon = categoryActionOptions[i].querySelector('i'); // ikona strzałki
            let options = categoryActionOptions[i].querySelector('.category-action-options'); // lista opcji w danym wierszu - "Przelądaj", "Zmień status", "Archiwizuj"
